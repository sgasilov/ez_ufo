...
def parse_args():
    ...
    parser.add_argument('--tmpdir', default='/data/tmp-stitch_search/', type=str,
                        help='Temporary directory for intermediate steps')
    ...
    parser.add_argument('--axis_on_left', default=True, type=bool,
                        help='Is the rotation axis on the left-hand side of the image?')
...
def find_overlap(args):
    ...
    for filename in os.listdir(os.path.join(proc, 'sinos')):
        if '.tif' in filename:
            ...
            recon_cmd = 'tofu tomo  --output-bytes-per-file 0 --sinograms ' + os.path.join(proc, 'sinos',
                                                                                           filename) + ' --output ' + os.path.join(
                output, filename) + ' --axis ' + str(axis)
            os.system(recon_cmd)
